...
COPY GLOBAL_MARKET_IMPACT_RADAR_RENDER_LS_200_COMPOSITE_TOP10.bin /tmp/app.tar.gz
RUN set -eux; \
    echo "Using release archive: GLOBAL_MARKET_IMPACT_RADAR_RENDER_LS_200_COMPOSITE_TOP10.bin"; \
...
USER radar
EXPOSE 3000
HEALTHCHECK --interval=30s --timeout=5s --start-period=30s --retries=3 CMD wget -qO- http://127.0.0.1:${PORT:-3000}/api/health >/dev/null || exit 1
CMD ["node", "server.js"]
